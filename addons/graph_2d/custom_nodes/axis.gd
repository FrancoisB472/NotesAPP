...
func _draw() -> void:
	...
	if show_x_numbers:
		for grad in hor_grad:
			draw_string(default_font, grad[POINT] + Vector2(0, 20), grad[LABEL],HORIZONTAL_ALIGNMENT_CENTER,0,FONT_SIZE)
	...
	if show_y_numbers == true:
		for grad in vert_grad:
			draw_string(default_font, grad[0] + Vector2(-35, -5), grad[1],HORIZONTAL_ALIGNMENT_CENTER,0,FONT_SIZE)
