...
func _init_theme():
	...
	if ThemeLibrary.themes.is_empty():
		ThemeLibrary.create_new_theme()

	ThemeLibrary.select_theme(ThemeLibrary.current_index if ThemeLibrary.current_index >= 0 else 0)

func apply_texture(tex: Texture2D) -> void:
	var data := ThemeLibrary.get_current()
	if data == null:
		return
	data.container_texture = tex
	ThemeManager.apply_game_theme(data)
